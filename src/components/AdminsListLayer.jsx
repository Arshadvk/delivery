...
const AdminsListLayer = ({ user }) => {
  ...
  const fetchAdmins = () => {
    ...
    if (token) {
      ...
      axios
        .get(
          `https://logistics.nicheperfumery.ae/user/list-admins?page=${currentPage}&limit=${limit}&search=`,
          {
            headers: {
  ...
  const handleSearch = (e) => {
    ...
    if (token) {
      axios
        .get(
          `https://logistics.nicheperfumery.ae/user/list-admins?page=1&limit=10&search=${searchTerm}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
                    ...
                    {sortedUsers.map((user, index) => (
                      <tr key={user.id}>
                        ...
                        <td className="text-center">
                          <span
                            className={`mt-3 px-24 py-4 radius-4 fw-medium text-sm border ${
